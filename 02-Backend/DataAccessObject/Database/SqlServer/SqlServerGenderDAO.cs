// *****************************************************************************************************
// Descripción       : Clase que define las acciones de la base de datos SQLServer para los generos
...
{
    public class SqlServerGenderDAO : BaseDAO, IGenderRepository
    {
        ...
        // Constructor
        public SqlServerGenderDAO(string strConexion, SqlServerLogDAO clsLogDao)
        {
            strCadenaConexion = strConexion;
            this.clsLogDao = clsLogDao;
        }

        public async Task<OperationResponse> Search(Gender bGenero, Log bLog)
        {
            // Declaración de variables
            ...
            try
            {
                AddParameter(parameters, "@P_GENCOD", !string.IsNullOrEmpty(bGenero.GenCod) ? bGenero.GenCod : DBNull.Value);
                AddParameter(parameters, "@P_GENNAM", !string.IsNullOrEmpty(bGenero.GenNam) ? bGenero.GenNam : DBNull.Value);

                strSql = "SP_SEARCH_GENDER";
                ...
                {
                    var dtsData = DeserializeDataSet<List<Gender>>(dtsDatos) ?? new List<Gender>();
