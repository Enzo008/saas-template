...
BEGIN TRY
    ...
    -- Obtener datos antes de la modificación con JOIN si aplica
    SET @V_SQL = ' SELECT @V_CADENA = ''EL [ENTITY] ['' + ENT.[FIELD1] + '']'' ' +
    ...
    SET @V_SQL = ' SELECT @V_CANTIDAD_REGISTRO = COUNT(*) ' +
                 ...
                 ' AND ([ENTITY]ANO <> ''' + LTRIM(RTRIM(@P_[ENTITY]ANO)) + ''' ' +
                 ' OR [ENTITY]COD <> ''' + LTRIM(RTRIM(@P_[ENTITY]COD)) + ''') ' +
                 ' AND ESTREG <> ''E'' '
    ...
    SET @V_SQL = ' UPDATE ' + @V_NAME_TABLE + ' SET ' +
                 ...
                 -- ' [RELATED_COD] = ''' + LTRIM(RTRIM(UPPER(@P_[RELATED_COD]))) + ''', ' +
    ...
    -- Obtener datos después de la modificación con JOIN si aplica
    SET @V_SQL_MODIFICADO = ' SELECT @V_CADENA_MODIFICADO = ''EL [ENTITY] ['' + ENT.[FIELD1] + '']'' ' +
                            ' FROM ' + @V_NAME_TABLE + ' AS ENT ' +
                            -- ' INNER JOIN TB_[RELATED_TABLE] AS REL ON ENT.[RELATED_COD] = REL.[RELATED_COD] ' +
                            ' WHERE ENT.[ENTITY]ANO = ''' + LTRIM(RTRIM(@P_[ENTITY]ANO)) + ''' ' +
                            ' AND ENT.[ENTITY]COD = ''' + LTRIM(RTRIM(@P_[ENTITY]COD)) + ''' '
...
-- 5. Descomentar líneas de campos relacionados y JOINs si aplica
