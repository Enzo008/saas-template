...
BEGIN TRY
    ...
    BEGIN
        SET @P_DESCRIPCION_MENSAJE  = 'The repository already exists in the database'
    ...
    -- Generate code
    EXEC SP_GENERAR_CODIGO @V_DEFECTO, 'TM_REPOSITORY', 'REPCOD', @V_CODIGO = @V_CODIGO_GENERADO OUTPUT
    ...
    SET @P_REPYEA_OUT = @V_YEAR
    SET @P_REPCOD_OUT = @V_CODIGO_GENERADO

    -- Generate success message
    SET @V_SQL_MENSAJE = '  SELECT ' + 
                         '  @V_CADENA = ''REPOSITORY ['' + REP.REPNAM + '']'' ' +
                         '  FROM TM_REPOSITORY AS REP ' +
                         '  WHERE REP.REPYEA = ''' + @V_YEAR + ''' AND REP.REPCOD = ''' + @V_CODIGO_GENERADO + ''''
    ...
    SET @P_DESCRIPCION_MENSAJE  = 'Repository created successfully'
